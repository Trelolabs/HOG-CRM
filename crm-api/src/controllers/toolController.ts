...
const ALLOWED_STATUSES = ['NEW', 'QUALIFIED', 'CONTACTED', 'CLOSED'] as const;
...
export const importCSV = async (req: Request, res: Response) => {
    ...
    try {
        ...
        rows.forEach((row, index) => {
            ...
            if (!ALLOWED_STATUSES.includes(statusValue as (typeof ALLOWED_STATUSES)[number])) {
                errors.push({
                    row: index + 2,
            ...
            preparedRows.push({
                ...
                status: statusValue as (typeof ALLOWED_STATUSES)[number]
            });
        });
